...
from src.uninstall.manager import UninstallManager
from src.uninstall.names import name_matches
...
def deterministic_gram_buckets(monkeypatch):
    ...
    (see ``ResidueEntryIndex._gram_bucket``). That is correct there, but any test
...
def test_run_uninstall_closes_the_whole_selection_before_removing_any():
    ...
    with (
        ...
        patch("src.ui.screens.uninstall.UninstallPreviewSelector.run", return_value=True),
        patch("src.uninstall.residue.find_residue_paths", return_value=[]),
        patch("src.uninstall.processes.candidate_process_names", return_value=["test"]),
        patch("src.uninstall.manager.running_process_comms", return_value={"test": [4242]}),
        patch(
            "src.uninstall.processes.terminate_apps",
...
def test_terminate_apps_waits_once_for_the_whole_selection(mock_sleep):
    ...
    with (
        patch("src.uninstall.processes.running_process_comms", side_effect=tables),
        patch(
            "src.uninstall.processes.candidate_process_names",
            side_effect=[["editor", "editor"], ["player"]],
        ),
        patch(
            "src.uninstall.processes.system.run_command", return_value=MagicMock(ok=True)
        ) as mock_run_cmd,
    ):
        processes.terminate_apps(targets)
...
def test_terminate_apps_does_not_wait_for_a_kill_it_never_sent(mock_sleep):
    ...
    with (
        patch("src.uninstall.processes.running_process_comms", side_effect=[{"editor": [11]}, {}]),
        patch(
            "src.uninstall.processes.candidate_process_names",
            return_value=["editor"],
        ),
        ...
        processes.terminate_apps([(_ui_app("DNF"), [], True)])
...
def test_terminate_apps_does_not_wait_when_nothing_is_running(mock_sleep):
    ...
    with (
        patch("src.uninstall.processes.running_process_comms", return_value={"unrelated": [1]}),
        patch(
            "src.uninstall.processes.candidate_process_names",
            return_value=["editor"],
        ),
        ...
        processes.terminate_apps([(_ui_app("DNF"), [], False)])
...
def test_run_uninstall_execute_and_exit():
    ...
    with (
        ...
        patch("src.ui.screens.uninstall.UninstallSelector.run", return_value=[0]),
        patch("src.ui.screens.uninstall.UninstallPreviewSelector.run", return_value=True),
        patch("src.uninstall.removal.execute_uninstall") as mock_exec,
        patch("src.ui.navigator.Navigator.wait_for_return", return_value=False),
...
def test_find_residue_paths(test_env):
    ...
    with patch("pathlib.Path.home", return_value=test_env):
        paths = residue.find_residue_paths("myapp", "MyApp")
...
def test_residue_shared_indexes_are_scanned_once(test_env):
    ...
    with patch("pathlib.Path.home", return_value=test_env):
        index = residue.pre_scan_search_roots()
        with (
            ...
            patch(
                "src.uninstall.residue.os.scandir", side_effect=AssertionError("rescanned roots")
            ),
        ):
            paths = residue.find_residue_paths(
                "com.example.myapp", "MyApp", pre_scanned_entries=index
            )
...
def _index_matches(index, targets):
    """What find_residue_paths() keeps: candidates that survive name_matches()."""
    return [
        entry
        for entry in index.candidates(targets)
        if any(name_matches(entry[0], target) for target in targets)
    ]
...
def _pad_to_indexed(entries, filler_name, test_env):
    ...
    """
    needed = ResidueEntryIndex._MIN_ENTRIES_TO_INDEX - len(entries)
...
def test_residue_index_candidates_preserve_name_match_semantics(test_env):
    ...
    targets = {"myapp", "go"}
    expected = [
        entry for entry in entries if any(name_matches(entry[0], target) for target in targets)
    ]

    small = ResidueEntryIndex.build(entries)
    ...
    large = ResidueEntryIndex.build(_pad_to_indexed(entries, "filler-", test_env))
...
def test_residue_index_reduces_full_index_matching(test_env, deterministic_gram_buckets):
    ...
    matches = [entry for entry in candidates if name_matches(entry[0], "myapp")]
...
def test_residue_index_narrows_when_noise_shares_the_target_prefix(test_env):
    ...
    that must still hold is the part that matters: name_matches() is the final
    ...
    assert _index_matches(index, {"myapp"}) == [
        entry for entry in entries if name_matches(entry[0], "myapp")
    ]
...
def test_residue_index_small_root_skips_the_gram_table(test_env):
    ...
    tiny = ResidueEntryIndex.build([("a.service", test_env / "a.service")])
...
def test_residue_index_is_compared_by_identity(test_env):
    ...
    assert index == index
    assert index != ResidueEntryIndex.build(entries)
...
def test_find_residue_paths_ignores_generic_short_tail_tokens(test_env):
    ...
    with patch("pathlib.Path.home", return_value=test_env):
        assert residue.find_residue_paths("org.example.go", "Example Go") == []
        assert residue.find_residue_paths("org.example.code", "Example Code") == []
        assert residue.find_residue_paths("org.example.id", "Example Id") == []
...
def test_find_residue_paths_allows_specific_prefix_and_substring(test_env):
    ...
    with patch("pathlib.Path.home", return_value=test_env):
        telegram_paths = residue.find_residue_paths("org.telegram.desktop", "Telegram")
        myapp_paths = residue.find_residue_paths("com.example.myapp", "MyApp")
...
def test_find_residue_paths_skips_official_only_apps(test_env):
    ...
    with patch("pathlib.Path.home", return_value=test_env):
        assert residue.find_residue_paths("tailscale", "Tailscale VPN") == []
        assert residue.find_residue_paths("org.fcitx.Fcitx5", "Fcitx5") == []
...
def test_execute_uninstall_keeps_data_when_the_package_removal_fails(
    ...
    with (
        patch("pathlib.Path.home", return_value=test_env),
        patch("src.uninstall.removal.safe_remove") as mock_remove,
    ):
        details = removal.execute_uninstall(app, [residue])
...
def test_sandbox_app_data_goes_to_the_trash_even_with_trash_disabled(
    ...
    with (
        patch("pathlib.Path.home", return_value=test_env),
        patch("src.uninstall.removal.get_use_trash", return_value=False),
        patch("src.uninstall.removal.safe_remove", return_value=(True, "OK")) as mock_remove,
    ):
        removal.execute_uninstall(app, [sandbox, snap_data, cache])
...
def test_execute_uninstall_dnf(mock_run, mock_run_cmd, test_env):
    ...
    with (
        ...
        patch("pathlib.Path.home", return_value=test_env),
        patch("src.uninstall.removal.safe_remove", return_value=(True, "OK")),
        patch("src.uninstall.processes.running_process_comms", return_value={"heavy-app": [4242]}),
    ):
        ...
        details = removal.execute_uninstall(app, [dummy_path])
...
def test_find_residue_paths_never_targets_xdg_user_dirs(test_env):
    ...
    with patch("pathlib.Path.home", return_value=test_env):
        music_paths = residue.find_residue_paths("org.gnome.Music", "Music")
        videos_paths = residue.find_residue_paths("org.gnome.Totem", "Videos")
        docs_paths = residue.find_residue_paths("com.example.Documents", "Documents")
...
def test_candidate_process_names_uses_ids_not_display_name():
    ...
    names = processes.candidate_process_names(app)
...
def test_candidate_process_names_ignores_unrelated_desktop_entries(test_env):
    ...
    with patch("pathlib.Path.home", return_value=test_env):
        names = processes.candidate_process_names({"id": "go", "name": "Go", "type": "DNF"})
...
def test_candidate_process_names_still_matches_boundary_and_reverse_dns(test_env):
    ...
    with patch("pathlib.Path.home", return_value=test_env):
        names = processes.candidate_process_names(
            {"id": "telegram", "name": "Telegram", "type": "DNF"}
        )
...
def test_find_residue_paths_skips_visible_home_workspace(test_env):
    ...
    with patch("pathlib.Path.home", return_value=test_env):
        paths = residue.find_residue_paths("com.example.notes", "Notes")
...
def test_execute_uninstall_residue_goes_to_trash(test_env):
    ...
    with (
        ...
        patch(
            "src.uninstall.removal.safe_remove", return_value=(True, "Moved to trash")
        ) as mock_safe_remove,
    ):
        removal.execute_uninstall(app, [residue])
...
def test_uninstall_helpers_and_cache_state(test_env, monkeypatch):
    mgr = UninstallManager()
    assert name_matches("firefox-profile", "firefox")
    assert not name_matches("app-data", "app")
    assert residue._requires_official_only_uninstall("org.example.vpn", "VPN")
...
def test_build_removal_targets_reuses_the_scan_index(monkeypatch):
    ...
    with (
        patch("src.uninstall.residue.find_residue_paths", side_effect=record),
        patch("src.uninstall.processes.candidate_process_names", return_value=[]),
    ):
...
def _collateral(app_type: str, stdout: str, app_id: str = "vlc"):
    ...
    with patch(
        "src.uninstall.collateral.system.run_command", return_value=MagicMock(stdout=stdout)
    ) as mock_run_cmd:
        names = collateral_packages({"id": app_id, "type": app_type})
...
def test_collateral_packages_previews_the_apt_transaction_that_will_run():
    ...
    with patch(
        "src.uninstall.collateral.system.run_command", return_value=MagicMock(stdout="")
    ) as query:
        collateral_packages(app)
    with (
        patch(
        ...
        patch("src.uninstall.processes.running_process_comms", return_value=set()),
    ):
        removal.execute_uninstall(app, [])

    preview_argv = query.call_args.args[0]
    removal_argv = next(
        call.args[0] for call in removal_run.call_args_list if call.args[0][0] == "apt-get"
    )
...
def test_build_targets_and_execute_cli_npm_and_systemd(test_env):
    ...
    with (
        patch("src.uninstall.residue.find_residue_paths", return_value=[]),
        patch("src.uninstall.processes.candidate_process_names", return_value=["tool"]),
    ...
    with (
        ...
        patch("src.uninstall.removal.shutil.which", return_value="/usr/bin/systemctl"),
        patch("src.uninstall.removal.safe_remove", return_value=(True, "ok")),
    ):
        result = removal.execute_uninstall(app2, [service])
...
def test_npm_scope_dir_goes_when_the_uninstall_leaves_it_empty(test_env):
    ...
    with (
        ...
        assert removal.execute_uninstall(app, [])["package_removed"] is True
...
def test_run_uninstall_reports_apps_already_removed_before_a_ctrl_c(capsys):
    ...
    with (
        ...
        patch("src.ui.screens.uninstall.UninstallPreviewSelector.run", return_value=True),
        patch("src.uninstall.residue.find_residue_paths", return_value=[]),
        patch("src.uninstall.removal.execute_uninstall", side_effect=results),
        patch("src.core.system.ensure_sudo_session", return_value=True),
