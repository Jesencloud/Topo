...
*   **系统底层组件与系统预装应用全自动精准防护**:
    *   在 `_SYSTEM_COMPONENT_TOKENS` 和 `_SYSTEM_COMPONENT_EXACT_IDS` 中补充了 `gcc`、`g++`（核心 C/C++ 编译器套件）以及所有 GNOME / Fedora 默认系统预装套件（包含 `gnome-calculator`、`gnome-calendar`、`gnome-clocks`、`gnome-weather`、`gnome-maps`、`gnome-boxes`、`loupe`、`snapshot`、`showtime`、`decibels`、`baobab`、`orca`、`papers`、`yelp` 等全部默认自带软件），确保 Uninstall 列表中只展现用户主动安装的第三方应用。
