# Daily Modification Report - 2026-08-15

## Project: topo (Topo) - 终端颜色模式、状态展示与菜单交互修复

本次修改修复了 `--no-color` 在模块导入后无法传播到 UI 消费方的问题，改善了无色终端中的进度条可读性，并补齐主菜单数字键导航及电池状态详情格式。帮助文本同时明确了无色模式的实际边界：报告与状态视图禁用颜色，交互菜单仍保留少量不影响可用性的装饰性高亮。全量门控通过，共计 392 项 Python 测试与 9 项 Rust 测试。

### 1. `--no-color` 运行时传播修复

* 在 `src/core/constants.py` 中集中维护颜色符号清单，并在 `setup_color_mode()` 更新颜色后，将新值传播到已经通过 `from constants import ...` 导入颜色值的 Topo 模块。
* 修复 argparse 解析完成后再启用 `--no-color` 时，各 UI 模块仍持有旧 ANSI 颜色值的问题；同时保持 `NO_COLOR` 环境变量和非 TTY 自动无色逻辑。
* 在 `src/main.py` 中收紧 `--no-color` 帮助说明，准确声明报告与状态视图禁用颜色，而交互菜单仍保留少量装饰性高亮，避免对功能范围作出过度承诺。

### 2. 无色进度条与边界处理

* `src/ui/navigator.py` 的进度条在有色模式下继续使用原有 `▬` 字形；无色模式下使用 `▬` 表示填充、`─` 表示空轨道，使 0%、50% 和 100% 不再显示为相同的满条。
* 对填充宽度增加 `0..width` 钳位，负数或超过 100% 的输入不会产生错误宽度。
* 修复 0% 时丢弃 `force_color` 的问题：调用方显式指定颜色时保留警示色，未指定时仍使用中性灰色，避免 Swap/ZRAM 的空条被误读为绿色满条。

### 3. 菜单与状态显示修复

* `InteractiveMenu` 支持 `1` 到 `9` 数字键直接选择并激活对应菜单项；`0` 和越界数字保持忽略。
* 修复电池详情的重复括号，状态行由 `((Health: 100.0%) | Cycles: 244)` 调整为 `(Health: 100.0% | Cycles: 244)`。

### 4. 回归测试与质量门控

* 新增颜色模式传播测试，验证 `--no-color` 能更新已导入颜色常量的 UI 与状态模块。
* 新增无色进度条层级区分、宽度边界、0% 强制颜色及有色模式字形兼容测试。
* 新增主菜单数字键选择、越界输入和零键忽略测试，以及电池详情无重复括号测试。
* `./check.sh` 全部通过：Ruff、Mypy、Tach、Vulture、Cargo fmt、Clippy、ShellCheck、392 项 Python 测试和 9 项 Rust 测试均通过。
