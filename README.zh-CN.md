...
| 键 | 默认值 | 含义 |
| --- | --- | --- |
| `use_trash` | `true` | 一次**可恢复**的删除去哪儿。编辑器备份、孤儿缓存残留、应用残留、失效的自动启动项、断链的用户软链接，以及 `topo analyze` 中删除的用户自有路径会进回收站；改成 `false` 则直接抹掉。不可再生的应用数据（包括 `~/.var/app` 与 `~/snap` 下的沙盒数据）始终进入回收站。常规缓存／临时文件清理始终永久删除，`topo analyze` 中需要管理员权限的删除也始终不可恢复。 |
